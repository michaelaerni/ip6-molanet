...
def conv2d(features, feature_count, name, use_batchnorm=True, strides=None, filter_sizes=None, do_activation=True, padding="SAME", input_channels=None):
    ...
    conv = tf.nn.bias_add(tf.nn.conv2d(features, w, strides=[1, strides[0], strides[1], 1], padding=padding, name=f"conv_{name}"), b)
    if use_batchnorm:
...
def conv2d_transpose(features, feature_count, output_size, name, keep_prob, batch_size, strides=None, filter_sizes=None, concat_activations=None, use_batchnorm=True, do_activation=True, padding="SAME"):
    ...
    b = bias_variable("b_" + name, [feature_count])
    conv = tf.nn.bias_add(tf.nn.conv2d_transpose(features, w, output_shape=[batch_size, output_size[0], output_size[1], feature_count], strides=[1, strides[0], strides[1], 1], padding=padding, name=f"conv_trans_{name}"), b)
    if use_batchnorm:
        bn = tf.contrib.layers.batch_norm(conv, decay=0.9, epsilon=1e-5)  # TODO: Params?
    else:
        bn = conv
...
class MolanetPoc(object):
    ...
    def _create_generator(image: tf.Tensor, keep_prob: tf.Tensor=tf.constant(0.5), reuse=False, reshape=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            a_enc3, _, _ = conv2d(a_enc2, 128, "enc_3", padding="VALID")
            a_enc4, _, _ = conv2d(a_enc3, 128, "enc_4", padding="VALID")
            a_enc5, _, _ = conv2d(a_enc4, 128, "enc_5", padding="VALID")

            a_dec1, _, _ = conv2d_transpose(a_enc5, 128, [4, 4], "dec_1", keep_prob, batch_size, concat_activations=a_enc4, use_batchnorm=False, padding="VALID")
            a_dec2, _, _ = conv2d_transpose(a_dec1, 128, [10, 10], "dec_2", keep_prob, batch_size, concat_activations=a_enc3, padding="VALID")
            ...
            logits, _, _ = conv2d(a_dec5, 1, "dec_out", strides=[1, 1], filter_sizes=[5, 5], use_batchnorm=False, do_activation=False, padding="VALID", input_channels=4)
            logits = tf.slice(logits, [0, 10, 10, 0], [batch_size, 70, 70, 1], name="crop")
    ...
    def _create_discriminator(image: tf.Tensor, mask: tf.Tensor, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            # 70x70x4 -> 68x68x64
            a1_1, w1_1, b1_1 = conv2d(input_tensor, 64, "disc_1_1", strides=[1, 1], filter_sizes=[3, 3], use_batchnorm=False, padding="VALID", input_channels=4)

            # 68x68x64 -> 64x64x64
            a1_2, w1_2, b1_2 = conv2d(a1_1, 64, "disc_1_2", strides=[1, 1], filter_sizes=[5, 5], padding="VALID")

            # 64x64x64 -> 32x32x128
            a2, w2, b2 = conv2d(a1_2, 128, "disc_2")

            # 32x32x128 -> 16x16x256
            a3, w3, b3 = conv2d(a2, 256, "disc_3")

            # 16x16x256 -> 8x8x512
            a4, w4, b4 = conv2d(a3, 256, "disc_4")

            # 8x8x512 -> 4x4x512
            a5, w5, b5 = conv2d(a4, 256, "disc_5")

            # 4x4x512 -> 2x2x512
            a6, w6, b6 = conv2d(a5, 256, "disc_6")
...
if __name__ == "__main__":
    ...
    l1_lambda = 0.001
    l1_loss = tf.reduce_sum(tf.abs((tf.round((gen_net + 1.0) / 2.0) - 0.5) * 2.0 - mask_patches))

    loss_gen = l1_lambda * l1_loss + tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake_logits, labels=tf.ones_like(disc_fake_logits)))
    ...
    with tf.Session() as sess:
        ...
        patch_count = patches_image.shape[0]
        split_count = 5
        batch_size = int(math.ceil(patch_count / split_count))

        assert patches_image.shape[0] == patches_segmentation.shape[0] and patches_segmentation.shape[0] == patches_gen_image.shape[0]
        ...
        for epoch in range(3000):
            ...
            for i in range(split_count):
                offset = i * batch_size
                count = batch_size if offset + batch_size <= patch_count else patch_count - offset
                _, cost_disc2 = sess.run([update_disc, loss_disc], feed_dict={
                    image_patches: patches_image[offset:offset+count],
                    image: patches_gen_image[offset:offset+count],
                    mask_patches: patches_segmentation[offset:offset+count]})
                cost_disc += cost_disc2

            cost_gen = 0
            for i in range(split_count):
                offset = i * batch_size
                count = batch_size if offset + batch_size <= patch_count else patch_count - offset
                _, cost_gen2 = sess.run([update_gen, loss_gen], feed_dict={
                    image: patches_gen_image[offset:offset+count],
                    mask_patches: patches_segmentation[offset:offset+count],
                    image_patches: patches_image[offset:offset+count]})
